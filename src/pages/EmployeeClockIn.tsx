...
const EmployeeClockIn: React.FC = () => {
  ...
  useEffect(() => {
    setLeaveStartTime(`${startHour}:${startMin}`);
  }, [startHour, startMin]);

  useEffect(() => {
    setLeaveEndTime(`${endHour}:${endMin}`);
  }, [endHour, endMin]);
  ...
  const leaveBalance = useMemo(() => {
    ...
    const usedDays = (type: string) => {
      const totalHours = approvedLeaves
    ...
    const annualUsedDays = Math.ceil((annualTotalHours - annualRemainingHours) / 8);

    return {
      annual:      { 
        total: annualTotalHours / 8, 
        used: annualUsedDays, 
        remaining: Math.max(0, (annualTotalHours / 8) - annualUsedDays), 
                ...
                <form onSubmit={handleSubmitLeave} style={{ display: 'flex', flexDirection: 'column', gap: '14px', marginBottom: '24px' }}>
                  <div style={{ background: 'linear-gradient(135deg,rgba(79,70,229,0.06),rgba(124,58,237,0.04))', borderRadius: '12px', padding: '16px', border: '1px solid rgba(79,70,229,0.1)' }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                      ...
                      <div>
                        <label style={labelStyle}>請假時數 (自動計算)</label>
        ...
        <div style={{
          ...
          <div className="glass-card" style={{
            ...
            <form onSubmit={handleUpdateLeave} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                <label style={{ fontSize: '13px', fontWeight: '600' }}>請假時數 (自動計算)</label>
                <input 
                  type="number" 
                  disabled 
